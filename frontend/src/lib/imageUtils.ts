  ...
  return new Promise((resolve) => {
    ...
    reader.onload = function(e) {
      ...
      img.onload = function() {
        ...
        }, file.type, 0.95); // Usa qualità 95% per mantenere buona qualità
      };
 ...
 */
function readOrientationFromExif(dataView: DataView, offset: number): number | null {
  ...
  const firstIFDOffset = littleEndian ? dataView.getUint32(offset + 4, true) : dataView.getUint32(offset + 4, false);
 ...
 */
export function resizeImage(file: File, maxWidth: number = 1200, maxHeight: number = 1200, quality: number = 0.8): Promise<File> {
  return new Promise((resolve) => {
    if (!file.type.startsWith('image/')) {
      resolve(file);
      return;
